...
import {
  ...
  importCustomerRemittanceBankResponseCsvSchema
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const payload = importCustomerRemittanceBankResponseCsvSchema.safeParse(body);
  ...
  const idempotencyScope = "treasury.remittances.import-bank-response-csv.v1";
  const result = await importCustomerRemittanceBankResponseCsv(
    params.remittanceId,
    payload.data,
    authorization.user,
    {
