...
import {
  ...
  createCustomerRemittanceDraftSchema,
...
export async function GET(request: Request) {
  ...
  const payload = listCustomerRemittancesSchema.safeParse({
  ...
  return jsonResponse(
    request,
    await listCustomerRemittances(payload.data, authorization.user)
  );
}
...
export async function POST(request: Request) {
  ...
  const payload = createCustomerRemittanceDraftSchema.safeParse(body);

  if (!payload.success) {
    return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  }

  const result = await createCustomerRemittanceDraft(payload.data, authorization.user, {
    correlationId
  });
