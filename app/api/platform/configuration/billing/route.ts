...
import {
  ...
  updateBillingConfigurationSchema
} from "@/modules/billing/application/configuration";
import {
  requirePermission,
  sessionCookieName,
  validateCsrfToken
...
export async function PATCH(request: Request) {
  ...
  const maintenance = await requireMaintenanceModeInactive(authorization.user, request, {
    correlationId
  });
  ...
  const payload = updateBillingConfigurationSchema.safeParse(body);

  if (!payload.success) {
    return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  }

  const result = await updateBillingConfiguration(payload.data, authorization.user);
