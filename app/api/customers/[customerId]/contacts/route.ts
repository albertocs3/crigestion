...
export async function GET(
  ...
  const auth = await requirePermission(token, "Customers.View", {
    correlationId,
  });
  ...
  const result = await listCustomerContacts(params.data.customerId, auth.user);
  return result
...
export async function POST(
  ...
  const result = await createCustomerContact(
    ...
    auth.user,
    {
      ...
      requestHash: hashCustomerContactRequest(parsed.data),
      scope: `customer:${params.data.customerId}:contact:create`,
      correlationId,
    },
  );
  return result.ok
