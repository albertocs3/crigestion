...
import {
  updateCustomerStore,
  updateCustomerStoreSchema,
  updateCustomerStoreStatus,
  updateCustomerStoreStatusSchema
...
const bodySchema = z.discriminatedUnion("action", [
  z.object({
    ...
    store: updateCustomerStoreSchema
  }).strict(),
  ...updateCustomerStoreStatusSchema.options
]);

export async function PATCH(
  request: Request,
  context: { params: Promise<{ customerId: string; storeId: string }> }
) {
  ...
  const result = payload.data.action === "update"
    ...
    : await updateCustomerStoreStatus(
        params.data.customerId,
        params.data.storeId,
        payload.data,
        authorization.user,
