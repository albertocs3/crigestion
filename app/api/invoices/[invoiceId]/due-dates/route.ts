...
import { replaceInvoiceDueDates, replaceInvoiceDueDatesSchema } from "@/modules/billing/application/invoices";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";
const paramsSchema = z.object({ invoiceId: z.string().uuid() });

export async function PUT(request: Request, context: { params: Promise<{ invoiceId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Billing.ManageDrafts", { correlationId });
  ...
  const payload = replaceInvoiceDueDatesSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await replaceInvoiceDueDates(params.data.invoiceId, payload.data, authorization.user, { correlationId });
