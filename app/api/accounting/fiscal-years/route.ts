...
import { createAccountingFiscalYearSchema, createInitialAccountingFiscalYear, listAccountingFiscalYears } from "@/modules/accounting/application/fiscalYears";
...
export async function GET(request: Request) {
  ...
  const authorization = await requirePermission(token, "Accounting.View", { correlationId: getCorrelationId(request) });
  if (!authorization.ok) return jsonResponse(request, authorization.error, { status: authorization.status });
  return jsonResponse(request, await listAccountingFiscalYears());
}
...
export async function POST(request: Request) {
  ...
  const authorization = await requirePermission(token, "Accounting.ManageExercises", { correlationId });
  if (!authorization.ok) return jsonResponse(request, authorization.error, { status: authorization.status });
  ...
  const payload = createAccountingFiscalYearSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await createInitialAccountingFiscalYear(payload.data.year, authorization.user, { correlationId });
