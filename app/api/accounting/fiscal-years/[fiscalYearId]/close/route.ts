...
export async function POST(request: Request, context: RouteContext) {
  ...
  const { fiscalYearId } = params.data;
  const result = await closeAccountingFiscalYear(fiscalYearId, authorization.user, {
    correlationId,
    ...
    requestHash: hashAccountingFiscalYearClose(fiscalYearId)
  });
