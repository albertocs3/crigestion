...
import { z } from "zod";
import { cancelFiscalYearCloseRequest, hashFiscalYearCloseCancellation } from "@/modules/accounting/application/fiscalYearCloseRequests";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const result = await cancelFiscalYearCloseRequest(params.data.requestId, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-fiscal-year-close-cancel", params.data.requestId, idempotency.key),
    requestHash: hashFiscalYearCloseCancellation(params.data.requestId)
  });
