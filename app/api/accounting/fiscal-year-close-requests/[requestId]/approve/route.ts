...
export async function POST(request: Request, context: RouteContext) {
  ...
  const authorization = await requirePermission(token, "Accounting.ApproveExerciseClosures", { correlationId });
  ...
  const result = await approveAccountingFiscalYearCloseRequest(params.data.requestId, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-fiscal-year-close-approve", params.data.requestId, idempotency.key),
    requestHash: hashAccountingFiscalYearCloseApproval(params.data.requestId)
  });
