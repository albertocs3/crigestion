...
import { cancelWaiverEvidenceReplacement, cancelWaiverEvidenceReplacementSchema, hashWaiverEvidenceReplacementCancellation } from "@/modules/accounting/application/waiverEvidenceReplacements";
...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  const payload = cancelWaiverEvidenceReplacementSchema.safeParse(body); if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
