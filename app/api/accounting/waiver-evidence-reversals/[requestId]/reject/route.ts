...
import { hashWaiverEvidenceReversalRejection, rejectWaiverEvidenceReversal, rejectWaiverEvidenceReversalSchema } from "@/modules/accounting/application/waiverEvidenceReversals";
...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Accounting.ApproveWaiverEvidenceReversals", { correlationId });
  ...
  const raw = await readBoundedTextBody(request, 2_048); if (raw === null) return jsonResponse(request, { code: "PAYLOAD_TOO_LARGE", message: "La petición supera el tamaño permitido." }, { status: 413 });
  let body: unknown; try { body = JSON.parse(raw); } catch { return jsonResponse(request, invalidJson(), { status: 400 }); }
  const payload = rejectWaiverEvidenceReversalSchema.safeParse(body); if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await rejectWaiverEvidenceReversal(params.data.requestId, payload.data, authorization.user, { correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-waiver-evidence-reversal-reject", params.data.requestId, idempotency.key),
    requestHash: hashWaiverEvidenceReversalRejection(params.data.requestId, payload.data) });
