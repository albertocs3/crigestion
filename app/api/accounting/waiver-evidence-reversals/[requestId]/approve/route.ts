...
import {
  approveWaiverEvidenceReversal, approveWaiverEvidenceReversalSchema, hashWaiverEvidenceReversalApproval
...
export async function POST(request: Request, context: { params: Promise<{ requestId: string }> }) {
  ...
  const raw = await readBoundedTextBody(request, 1_024);
  if (raw === null) return jsonResponse(request, { code: "PAYLOAD_TOO_LARGE", message: "La petición supera el tamaño permitido." }, { status: 413 });
  let body: unknown;
  try { body = JSON.parse(raw); } catch { return jsonResponse(request, invalidJson(), { status: 400 }); }
  const payload = approveWaiverEvidenceReversalSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await approveWaiverEvidenceReversal(params.data.requestId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-waiver-evidence-reversal-approve", params.data.requestId, idempotency.key),
    requestHash: hashWaiverEvidenceReversalApproval(params.data.requestId, payload.data)
  });
