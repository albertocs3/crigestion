...
import { z } from "zod";
import { approveFiscalYearReopening, hashFiscalYearReopenApproval } from "@/modules/accounting/application/fiscalYearReopenRequests";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const authorization = await requirePermission(token, "Accounting.ApproveExerciseReopenings", { correlationId });
  ...
  const result = await approveFiscalYearReopening(params.data.requestId, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-fiscal-year-reopen-approve", params.data.requestId, idempotency.key),
    requestHash: hashFiscalYearReopenApproval(params.data.requestId)
  });
