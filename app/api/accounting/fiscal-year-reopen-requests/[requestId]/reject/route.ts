...
import {
  ...
  rejectFiscalYearReopeningSchema
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const command = rejectFiscalYearReopeningSchema.safeParse(body);
  if (!command.success) return jsonResponse(request, validationError(command.error.flatten()), { status: 422 });
  const result = await rejectFiscalYearReopening(params.data.requestId, command.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-fiscal-year-reopen-reject", params.data.requestId, idempotency.key),
    requestHash: hashFiscalYearReopenRejection(params.data.requestId, command.data)
  });
