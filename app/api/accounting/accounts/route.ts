...
export async function GET(request: Request) {
  ...
  const payload = listAccountingAccountsSchema.safeParse({
  ...
  return jsonResponse(
    request,
    await listAccountingAccounts(payload.data, authorization.user)
  );
}
...
export async function POST(request: Request) {
  ...
  const payload = createAccountingAccountSchema.safeParse(body);

  if (!payload.success) {
    return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  }

  const result = await createAccountingAccount(payload.data, authorization.user, {
    correlationId
  });
