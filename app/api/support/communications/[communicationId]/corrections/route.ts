...
import {
  ...
  correctSupportCommunicationSchema,
...
export async function POST(
  ...
  const parsed = correctSupportCommunicationSchema.safeParse(body);
  if (!parsed.success)
    return response(request, validationError(parsed.error.flatten()), 422);
  const result = await correctSupportCommunication(
    ...
    auth.user,
    {
      ...
      scope: `communication:${params.data.communicationId}:correct`,
      correlationId,
    },
  );
  return result.ok
