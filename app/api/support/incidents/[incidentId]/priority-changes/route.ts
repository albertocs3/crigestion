...
import { changeSupportIncidentPriority, hashSupportPriorityChangeRequest, supportPriorityChangeSchema } from "@/modules/support/application/priorities";
...
export async function POST(request: Request, context: { params: Promise<{ incidentId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Support.ManageAssigned", { correlationId });
  ...
  const parsed = supportPriorityChangeSchema.safeParse(body);
  if (!parsed.success) return response(request, validationError(parsed.error.flatten()), 422);
  const result = await changeSupportIncidentPriority(params.data.incidentId, parsed.data, authorization.user, { idempotencyKey: idempotency.key, requestHash: hashSupportPriorityChangeRequest({ incidentId: params.data.incidentId, ...parsed.data }), scope: `incident:${params.data.incidentId}:priority-change`, correlationId });
