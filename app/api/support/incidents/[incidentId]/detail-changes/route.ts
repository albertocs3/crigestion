...
import { changeSupportIncidentDetails, hashSupportIncidentDetailsChangeRequest, supportIncidentDetailsChangeSchema } from "@/modules/support/application/detailsChanges";
...
export async function POST(request: Request, context: { params: Promise<{ incidentId: string }> }) {
  ...
  const parsed = supportIncidentDetailsChangeSchema.safeParse(body);
  if (!parsed.success) return response(request, validationError(parsed.error.flatten()), 422);
  const result = await changeSupportIncidentDetails(params.data.incidentId, parsed.data, authorization.user, {
    idempotencyKey: idempotency.key,
    requestHash: hashSupportIncidentDetailsChangeRequest({ incidentId: params.data.incidentId, ...parsed.data }),
    scope: `incident:${params.data.incidentId}:details-change`,
    correlationId,
  });
