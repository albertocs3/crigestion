...
import { hashSupportStatusTransitionRequest, supportStatusTransitionSchema, transitionSupportIncident } from "@/modules/support/application/statusTransitions";
...
export async function POST(request: Request, context: { params: Promise<{ incidentId: string }> }) {
  ...
  const maintenance = await requireMaintenanceModeInactive(viewAuthorization.user, request, { correlationId });
  ...
  const parsed = supportStatusTransitionSchema.safeParse(body);
  if (!parsed.success) return response(request, validationError(parsed.error.flatten()), 422);
  const requiredPermission = parsed.data.action === "reopen" ? "Support.Reopen" : "Support.ManageAssigned";
  ...
  const result = await transitionSupportIncident(params.data.incidentId, parsed.data, authorization.user, { idempotencyKey: idempotency.key, requestHash: hashSupportStatusTransitionRequest({ incidentId: params.data.incidentId, ...parsed.data }), scope: `incident:${params.data.incidentId}:status-transition`, correlationId });
