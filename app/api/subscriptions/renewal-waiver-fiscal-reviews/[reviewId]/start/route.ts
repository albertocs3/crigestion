...
import {
  hashStartRenewalWaiverFiscalReview, startRenewalWaiverFiscalReview,
  startRenewalWaiverFiscalReviewSchema
...
export async function POST(request: Request, context: { params: Promise<{ reviewId: string }> }) {
  ...
  const payload = startRenewalWaiverFiscalReviewSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await startRenewalWaiverFiscalReview(params.data.reviewId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-renewal-waiver-fiscal-review-start", params.data.reviewId, idempotency.key),
    requestHash: hashStartRenewalWaiverFiscalReview(params.data.reviewId, payload.data)
  });
