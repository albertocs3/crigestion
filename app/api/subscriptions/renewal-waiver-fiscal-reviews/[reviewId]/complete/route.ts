...
import {
  completeRenewalWaiverFiscalReview, completeRenewalWaiverFiscalReviewSchema,
...
export async function POST(request: Request, context: { params: Promise<{ reviewId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Subscriptions.CompleteRenewalWaiverFiscalReviews", { correlationId });
  ...
  const payload = completeRenewalWaiverFiscalReviewSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await completeRenewalWaiverFiscalReview(params.data.reviewId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-renewal-waiver-fiscal-review-complete", params.data.reviewId, idempotency.key),
    requestHash: hashCompleteRenewalWaiverFiscalReview(params.data.reviewId, payload.data)
  });
