...
import {
  hashWaiverEvidenceReversalRequest, requestWaiverEvidenceReversal, requestWaiverEvidenceReversalSchema
...
export async function POST(request: Request, context: { params: Promise<{ reviewId: string }> }) {
  ...
  const payload = requestWaiverEvidenceReversalSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await requestWaiverEvidenceReversal(params.data.reviewId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-waiver-evidence-reversal-request", params.data.reviewId, idempotency.key),
    requestHash: hashWaiverEvidenceReversalRequest(params.data.reviewId, payload.data)
  });
