...
import { hashWaiverEvidenceReplacementRequest, requestWaiverEvidenceReplacement, requestWaiverEvidenceReplacementSchema } from "@/modules/accounting/application/waiverEvidenceReplacements";
...
export async function POST(request: Request, context: { params: Promise<{ reviewId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Accounting.RequestWaiverEvidenceReplacements", { correlationId });
  ...
  const raw = await readBoundedTextBody(request, 131_072); if (raw === null) return jsonResponse(request, { code: "PAYLOAD_TOO_LARGE", message: "La petición supera el tamaño permitido." }, { status: 413 });
  let body: unknown; try { body = JSON.parse(raw); } catch { return jsonResponse(request, invalidJson(), { status: 400 }); }
  const payload = requestWaiverEvidenceReplacementSchema.safeParse(body); if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await requestWaiverEvidenceReplacement(params.data.reviewId, payload.data, authorization.user, { correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "accounting-waiver-evidence-replacement-request", params.data.reviewId, idempotency.key),
    requestHash: hashWaiverEvidenceReplacementRequest(params.data.reviewId, payload.data) });
