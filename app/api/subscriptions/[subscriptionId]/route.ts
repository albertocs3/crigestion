...
import { getSubscription, hashSubscriptionRequest, subscriptionParamsSchema, updateSubscription, updateSubscriptionSchema } from "@/modules/subscriptions/application/subscriptions";
...
export async function GET(request: Request, context: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Subscriptions.View", { correlationId });
  ...
  const subscription = await getSubscription(params.data.subscriptionId, authorization.user, { correlationId });
  return subscription
    ? jsonResponse(request, subscription)
    : jsonResponse(request, { code: "SUBSCRIPTION_NOT_FOUND", message: "La suscripcion no existe." }, { status: 404 });
}

export async function PUT(request: Request, context: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const correlationId = getCorrelationId(request);
  const authorization = await requirePermission(token, "Subscriptions.Manage", { correlationId });
  if (!authorization.ok) return jsonResponse(request, authorization.error, { status: authorization.status });
  const params = subscriptionParamsSchema.safeParse(await context.params);
  if (!params.success) return jsonResponse(request, validationError(params.error.flatten()), { status: 422 });
  ...
  const payload = updateSubscriptionSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await updateSubscription(params.data.subscriptionId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-update", params.data.subscriptionId, idempotency.key),
