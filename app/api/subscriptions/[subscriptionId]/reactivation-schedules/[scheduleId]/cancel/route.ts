...
import {
  hashSubscriptionReactivationScheduleRequest, revokeSubscriptionReactivationSchedule,
  revokeSubscriptionReactivationScheduleSchema, subscriptionReactivationScheduleParamsSchema
...
export async function POST(request: Request, context: { params: Promise<{ subscriptionId: string; scheduleId: string }> }) {
  ...
  const payload = revokeSubscriptionReactivationScheduleSchema.safeParse(body);
  ...
  const result = await revokeSubscriptionReactivationSchedule(params.data.subscriptionId, params.data.scheduleId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-reactivation-schedule-revoke", params.data.scheduleId, idempotency.key),
    requestHash: hashSubscriptionReactivationScheduleRequest("revoke", semanticPayload)
  });
