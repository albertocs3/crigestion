...
import { activateSubscription, activateSubscriptionSchema, hashSubscriptionRequest, subscriptionParamsSchema } from "@/modules/subscriptions/application/subscriptions";
...
export async function POST(request: Request, context: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const payload = activateSubscriptionSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await activateSubscription(params.data.subscriptionId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-activate", params.data.subscriptionId, idempotency.key),
