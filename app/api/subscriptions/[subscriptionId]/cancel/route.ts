...
import { cancelSubscription, cancelSubscriptionSchema, hashSubscriptionRequest, subscriptionParamsSchema } from "@/modules/subscriptions/application/subscriptions";
...
export async function POST(request: Request, context: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const payload = cancelSubscriptionSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await cancelSubscription(params.data.subscriptionId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-cancel", params.data.subscriptionId, idempotency.key),
