...
import { cancellationScheduleParamsSchema, cancelScheduledSubscriptionCancellation, cancelScheduledSubscriptionCancellationSchema, hashSubscriptionRequest } from "@/modules/subscriptions/application/subscriptions";
...
export async function POST(request: Request, context: { params: Promise<{ subscriptionId: string; scheduleId: string }> }) {
  ...
  const params = cancellationScheduleParamsSchema.safeParse(await context.params);
  ...
  const payload = cancelScheduledSubscriptionCancellationSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await cancelScheduledSubscriptionCancellation(params.data.subscriptionId, params.data.scheduleId, payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-cancellation-schedule-revoke", params.data.scheduleId, idempotency.key),
    requestHash: hashSubscriptionRequest({ ...params.data, ...payload.data })
  });
