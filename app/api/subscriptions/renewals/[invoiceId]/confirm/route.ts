...
import { confirmSubscriptionRenewal, currentSubscriptionRenewalCompanyId, hashSubscriptionRenewalConfirmationRequest } from "@/modules/subscriptions/application/renewals";
...
export async function POST(request: Request, context: { params: Promise<{ invoiceId: string }> }) {
  ...
  const authorization = await requirePermission(token, "Subscriptions.ConfirmRenewals", { correlationId });
  if (!authorization.ok) return jsonResponse(request, authorization.error, { status: authorization.status });
  const billingAuthorization = await requirePermission(token, "Billing.Issue", { correlationId });
  if (!billingAuthorization.ok) return jsonResponse(request, billingAuthorization.error, { status: billingAuthorization.status });
  ...
  const command = { companyId, invoiceId: params.data.invoiceId };
  const result = await confirmSubscriptionRenewal(command, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-renewal-confirm", params.data.invoiceId, idempotency.key),
    requestHash: hashSubscriptionRenewalConfirmationRequest(command)
