...
import { createSubscription, createSubscriptionSchema, hashSubscriptionRequest, listSubscriptions, listSubscriptionsSchema } from "@/modules/subscriptions/application/subscriptions";
...
export async function GET(request: Request) {
  ...
  const authorization = await requirePermission(token, "Subscriptions.View", { correlationId: getCorrelationId(request) });
  if (!authorization.ok) return jsonResponse(request, authorization.error, { status: authorization.status });
  const query = new URL(request.url).searchParams;
  const payload = listSubscriptionsSchema.safeParse({
    limit: query.get("limit") ?? undefined,
    cursor: query.get("cursor") ?? undefined,
    status: query.get("status") ?? undefined,
    periodicity: query.get("periodicity") ?? undefined,
    pricingMode: query.get("pricingMode") ?? undefined,
  ...
  return jsonResponse(request, await listSubscriptions(payload.data, authorization.user, { correlationId: getCorrelationId(request) }));
}
...
export async function POST(request: Request) {
  ...
  const payload = createSubscriptionSchema.safeParse(body);
  if (!payload.success) return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  const result = await createSubscription(payload.data, authorization.user, {
    correlationId,
    idempotencyKey: idempotencyStorageKey(authorization.user.id, "subscription-create", payload.data.customerId, idempotency.key),
    requestHash: hashSubscriptionRequest(payload.data)
  });
