...
import {
  ...
  createCatalogTaxRateSchema,
  listCatalogTaxRates,
  listCatalogTaxRatesSchema
...
export async function GET(request: Request) {
  ...
  const payload = listCatalogTaxRatesSchema.safeParse({
  ...
  return jsonResponse(request, { items: await listCatalogTaxRates(payload.data) });
}
...
export async function POST(request: Request) {
  ...
  const payload = createCatalogTaxRateSchema.safeParse(body);

  if (!payload.success) {
    return jsonResponse(request, validationError(payload.error.flatten()), { status: 422 });
  }

  const result = await createCatalogTaxRate(payload.data, authorization.user, {
    correlationId
  });
