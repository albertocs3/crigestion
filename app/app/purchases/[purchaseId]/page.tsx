...
import { PurchaseCorrectionReplaceForm, PurchaseCorrectionVoidForm, PurchaseDueDatesForm, PurchaseLinesForm, PurchaseRectificationForm, PurchaseRegisterButton } from "@/modules/purchases/presentation/PurchaseForms";
...
export default async function PurchasePage({ params }: { params: Promise<{ purchaseId: string }> }) {
  ...
  const canCorrect = authorization.user.permissions.includes("Purchases.Correct");
  const rectification = getPurchaseRectificationAvailability(purchase);
  const correction = getPurchaseCorrectionAvailability(purchase);
  const [items, taxRates] = editable || (canCorrect && correction.available)
  ...
  return <main className="shell">
    ...
    <section className="content stack">
      ...
      {canCorrect && correction.available ? <div className="panel stack"><PurchaseCorrectionVoidForm purchaseId={purchase.id} version={purchase.version} originalNumber={purchase.supplierInvoiceNumber} issueDate={purchase.issueDate}/></div> : null}
      {canCorrect && correction.available ? <div className="panel stack"><PurchaseCorrectionReplaceForm purchaseId={purchase.id} version={purchase.version} originalNumber={purchase.supplierInvoiceNumber} dates={{ issueDate: purchase.issueDate, receivedDate: purchase.receivedDate, operationDate: purchase.operationDate, accountingDate: purchase.accountingDate }} items={items.items.map((item) => ({ id: item.id, code: item.code, name: item.name, costPrice: item.costPrice, taxRateId: item.tax.id }))} taxRates={taxRates} existingLines={purchase.lines.map((line) => ({ catalogItemId: line.catalogItemId, description: line.description, quantity: line.quantity, unitPrice: line.unitPrice, discountPercent: line.discountPercent, discountAmount: line.discountAmount, purchaseAccountCode: line.purchaseAccountCode, taxRateId: line.taxRateId }))} existingDueDates={purchase.dueDates.map((due) => ({ dueDate: due.dueDate, amount: due.amount, paymentMethod: due.paymentMethod }))}/></div> : null}
