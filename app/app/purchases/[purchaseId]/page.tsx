...
import { PurchaseDueDatesForm, PurchaseLinesForm, PurchaseRegisterButton } from "@/modules/purchases/presentation/PurchaseForms";
...
export default async function PurchasePage({ params }: { params: Promise<{ purchaseId: string }> }) {
  ...
  return <main className="shell">
    ...
    <section className="content stack">
      <div className="panel stack"><div className="split-header"><div><p className="eyebrow">Compra</p><h1>{purchase.supplierInvoiceNumber}</h1><p className="muted">{purchase.supplierCode} - {purchase.supplierName}</p></div><span className="badge neutral">{purchase.status === "DRAFT" ? "Borrador" : "Registrada"}</span></div><div className="data-grid"><div><span className="data-label">Emisión</span><strong>{purchase.issueDate}</strong></div><div><span className="data-label">Contable</span><strong>{purchase.accountingDate}</strong></div><div><span className="data-label">Base</span><strong>{purchase.taxableBase} EUR</strong></div><div><span className="data-label">IVA</span><strong>{purchase.taxAmount} EUR</strong></div><div><span className="data-label">Total</span><strong>{purchase.total} EUR</strong></div><div><span className="data-label">Pago</span><strong>{purchase.paymentStatus}</strong></div></div>{purchase.accountingEntry ? <Link className="button button-secondary button-small" href={`/app/accounting?entryId=${purchase.accountingEntry.id}`}>Asiento {purchase.accountingEntry.number}</Link> : null}</div>
      {editable ? <>
      ...
      </> : <div className="panel stack"><h2>Vencimientos</h2><div className="table-wrap"><table><thead><tr><th>Fecha</th><th>Método</th><th className="numeric">Importe</th><th className="numeric">Pagado</th><th className="numeric">Pendiente</th><th>Estado</th></tr></thead><tbody>{purchase.dueDates.map((due) => <tr key={due.id}><td>{due.dueDate}</td><td>{due.paymentMethod}</td><td className="numeric">{due.amount}</td><td className="numeric">{due.allocatedAmount}</td><td className="numeric">{due.pendingAmount}</td><td>{due.status}</td></tr>)}</tbody></table></div><Link className="button" href={`/app/treasury/supplier-payments?supplierId=${purchase.supplierId}`}>Registrar pago</Link></div>}
    </section>
  </main>;
}
