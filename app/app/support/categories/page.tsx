...
export default async function SupportCategoriesPage() {
  ...
  return <main className="shell"><header className="topbar"><div className="brand">CriGestión</div><Link className="button button-secondary" href="/app/support">Volver</Link></header><section className="content stack"><div className="panel stack"><h1>Categorías de incidencias</h1><div className="table-wrap"><table><thead><tr><th>Nombre</th><th>Descripción</th><th>Color</th><th>Estado</th></tr></thead><tbody>{categories.map((category) => <tr key={category.id}><td><strong>{category.name}</strong></td><td>{category.description ?? "—"}</td><td><span aria-hidden="true" style={{ color: category.color }}>●</span> {category.color}</td><td>{category.isActive ? "Activa" : "Inactiva"}</td></tr>)}</tbody></table></div></div><div className="panel stack"><SupportCategoryCreateForm/></div></section></main>;
}
