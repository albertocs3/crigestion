...
export default async function SubscriptionDetailPage({ params }: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const businessDate = canReactivate && subscription.status === "CANCELLED" ? await subscriptionRenewalBusinessDate() : null;
  ...
  return <main className="shell">
    ...
    <section className="content stack">
      ...
      {canReactivate && subscription.status === "CANCELLED" && reactivationDateAvailable ? <div className="panel stack"><SubscriptionReactivateForm subscriptionId={subscription.id} version={subscription.version} defaultNextRenewalDate={defaultReactivationDate} minimumNextRenewalDate={minimumReactivationDate} maximumNextRenewalDate={subscription.endDate} /></div> : null}
      {canReactivate && subscription.status === "CANCELLED" && !reactivationDateAvailable ? <div className="panel stack"><h2>Reactivar suscripcion</h2><p className="message warning">No se puede reactivar porque la fecha final del contrato es anterior a la primera fecha de renovacion permitida.</p></div> : null}
      {canCancel && (subscription.status === "ACTIVE" || subscription.status === "RENEWAL_PENDING") ? <div className="panel stack"><SubscriptionCancelForm subscriptionId={subscription.id} version={subscription.version} /></div> : null}
      {canCancel && (subscription.status === "ACTIVE" || subscription.status === "RENEWAL_PENDING") ? <div className="panel stack"><SubscriptionCancellationScheduleForms subscriptionId={subscription.id} subscriptionVersion={subscription.version} nextRenewalDate={subscription.nextRenewalDate} pendingSchedule={pendingSchedule} /></div> : null}
