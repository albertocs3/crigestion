...
import { createIncidentCollaboratorAddedNotification, createIncidentReassignedNotification } from "@/modules/platform/application/notifications";
...
type Locked = { id: string; companyId: string; number: string; status: "NEW" | "IN_PROGRESS" | "PENDING_CUSTOMER" | "PENDING_THIRD_PARTY" | "RESOLVED" | "CLOSED"; version: number; responsibleUserId: string; mergedIntoIncidentId: string | null };
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const incident = (await tx.$queryRaw<Locked[]>(Prisma.sql`SELECT "id", "companyId", "number", "status", "version", "responsibleUserId", "mergedIntoIncidentId" FROM "support_incidents" WHERE "id" = ${incidentId}::uuid AND "companyId" = ${companyId}::uuid FOR UPDATE`))[0];
        if (!incident) return fail(404, "SUPPORT_INCIDENT_NOT_FOUND", "La incidencia no existe.");
        if (incident.mergedIntoIncidentId) return fail(409, "SUPPORT_INCIDENT_VERSION_CONFLICT", "Una incidencia fusionada no admite cambios de participantes.");
        ...
        const event = await tx.supportIncidentEvent.create({ data: { companyId, incidentId, actorUserId: actor.id, responsibleUserIdAtEvent: nextResponsible, participantChangeId: change.id, eventType: type, fromStatus: incident.status, toStatus: incident.status, resultingVersion }, select: { id: true } });
        if (type === "COLLABORATOR_ADDED" && command.action === "add-collaborator") await createIncidentCollaboratorAddedNotification(tx, { companyId, incidentId, sourceEventId: event.id, incidentNumber: incident.number, collaboratorUserId: command.userId, correlationId: context.correlationId });
