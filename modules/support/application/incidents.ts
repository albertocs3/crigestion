...
export type SupportIncidentDetail = SupportIncidentListItem & {
  ...
  collaborators: Array<{ id: string; user: { id: string; displayName: string }; addedAt: string; removedAt: string | null }>;
  participantChanges: Array<{ id: string; type: "COLLABORATOR_ADDED" | "COLLABORATOR_REMOVED" | "RESPONSIBLE_CHANGED"; reason: string | null; collaborator: { id: string; user: { id: string; displayName: string } } | null; fromResponsible: { id: string; displayName: string } | null; toResponsible: { id: string; displayName: string } | null; actor: { id: string; displayName: string }; occurredAt: string }>;
};
...
const incidentDetailSelect = {
  ...
  collaborators: { orderBy: [{ addedAt: "asc" as const }, { id: "asc" as const }], select: { id: true, addedAt: true, removedAt: true, user: { select: { id: true, displayName: true } } } },
  participantChanges: { orderBy: [{ occurredAt: "asc" as const }, { id: "asc" as const }], select: { id: true, changeType: true, reason: true, occurredAt: true, collaborator: { select: { id: true, user: { select: { id: true, displayName: true } } } }, fromResponsible: { select: { id: true, displayName: true } }, toResponsible: { select: { id: true, displayName: true } }, actorUser: { select: { id: true, displayName: true } } } }
} satisfies Prisma.SupportIncidentSelect;
...
const incidentReplaySchema: z.ZodType<SupportIncidentDetail> = z.object({
  ...
  collaborators: z.array(z.object({ id: z.string().uuid(), user: z.object({ id: z.string().uuid(), displayName: z.string() }).strict(), addedAt: z.string().datetime(), removedAt: z.string().datetime().nullable() }).strict()),
  participantChanges: z.array(z.object({ id: z.string().uuid(), type: z.enum(["COLLABORATOR_ADDED", "COLLABORATOR_REMOVED", "RESPONSIBLE_CHANGED"]), reason: z.string().nullable(), collaborator: z.object({ id: z.string().uuid(), user: z.object({ id: z.string().uuid(), displayName: z.string() }).strict() }).strict().nullable(), fromResponsible: z.object({ id: z.string().uuid(), displayName: z.string() }).strict().nullable(), toResponsible: z.object({ id: z.string().uuid(), displayName: z.string() }).strict().nullable(), actor: z.object({ id: z.string().uuid(), displayName: z.string() }).strict(), occurredAt: z.string().datetime() }).strict())
}).strict();
