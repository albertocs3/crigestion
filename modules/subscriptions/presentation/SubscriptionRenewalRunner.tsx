...
export function SubscriptionRenewalRunner({ preview, canConfirm, canExclude }: { preview: SubscriptionRenewalPreview; canConfirm: boolean; canExclude: boolean }) {
  ...
  function keyFor(action: string) {
    ...
    const value = crypto.randomUUID(); keys.current.set(action, value); return value;
  }
  async function mutate(path: string, body: unknown, action: string) {
    ...
    try {
      ...
      const response = await fetch(path, { method: "POST", headers: {
        "Content-Type": "application/json", "X-CSRF-Token": csrfToken, "Idempotency-Key": keyFor(action)
    ...
    <section className="panel stack">
      ...
      {preview.groups.length === 0 ? <p>No hay renovaciones disponibles para esta fecha.</p> : preview.groups.map((group) => {
        ...
        return <article className="panel stack" key={group.key}>
          ...
          <div className="table-wrap"><table><thead><tr><th scope="col">Incluir</th><th scope="col">Suscripción</th><th scope="col">Estado</th><th scope="col">Acción prevista</th><th scope="col">Estimación</th><th scope="col">Gestión</th></tr></thead><tbody>
          ...
          <button className="button" type="button" disabled={!group.selectable || selectedIds.length === 0 || busy !== null} onClick={() => {
            const subscriptions = group.subscriptions.filter((subscription) => selectedIds.includes(subscription.id)).map((subscription) => ({
