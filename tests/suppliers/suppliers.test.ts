...
import { approveAccountingFiscalYearCloseRequest, hashAccountingFiscalYearCloseApproval } from "@/modules/accounting/application/fiscalYears";
...
describe("suppliers application service", () => {
  ...
  it("uses optimistic concurrency and renames only accounts in open years", async () => {
    ...
    const firstYear = await prisma.accountingFiscalYear.findFirstOrThrow(); const closeRequest = await pendingCloseRequest(firstYear.id, actor.id); const closed = await approveAccountingFiscalYearCloseRequest(closeRequest.id, actor, { idempotencyKey: `test-supplier-close:${randomUUID()}`, requestHash: hashAccountingFiscalYearCloseApproval(closeRequest.id) }); expect(closed.ok).toBe(true);
...
async function pendingCloseRequest(fiscalYearId: string, approverId: string) { const year = await prisma.accountingFiscalYear.findUniqueOrThrow({ where: { id: fiscalYearId }, select: { companyId: true } }); const role = await prisma.role.findFirstOrThrow(); const suffix = randomUUID(); const requester = await prisma.user.create({ data: { displayName: "Solicitante cierre", userName: `close-${suffix}`, normalizedUserName: `close-${suffix}`, passwordHash: "test-only", roleId: role.id } }); if (requester.id === approverId) throw new Error("Invalid test actors."); return prisma.accountingFiscalYearCloseRequest.create({ data: { companyId: year.companyId, fiscalYearId, requestedById: requester.id, preflightSnapshot: { ready: true, journalEntryCount: 0, unbalancedEntryCount: 0, headerLineMismatchCount: 0, invalidEntryShapeCount: 0, invalidLineCount: 0, crossFiscalYearLineCount: 0, draftInvoiceCount: 0, invoiceWithoutEntryCount: 0, unresolvedVerifactuInvoiceCount: 0, draftPurchaseCount: 0, purchaseWithoutEntryCount: 0, pendingCustomerRefundCount: 0, pendingSupplierRefundCount: 0, unsupportedAccountBalanceCount: 0, resultAccountReady: true } } }); }
